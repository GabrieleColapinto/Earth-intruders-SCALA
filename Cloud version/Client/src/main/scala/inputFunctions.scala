package ops {
  ...
  class inputFunctions {
    ...
    // Input of the name of the player
    def inputName(): String = {
      print(s"Select the name of the player. (default = $DEFAULT_PLAYER): ")
      val playerName = scala.io.StdIn.readLine()
      if(playerName == ""){
        println(s"The name of the player has been set to $DEFAULT_PLAYER by default.")
        DEFAULT_PLAYER
      } else {
        playerName
      }
    ...
    def getDirection(stageMatrix: List[List[Int]], gameMode: Char, numRows: Int, numColumns: Int, playerColumn: Int): Char = {
      ...
      if(gameMode == GameModes.manual){
        ...
        movementFun.automaticDirection(stageMatrix, numRows, numColumns, playerColumn)
      }
    }
